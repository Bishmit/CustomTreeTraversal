...
void Game::showIntroScreen() {
    ...
    sf::Text titleText("Controls:", font, 50);
    titleText.setFillColor(sf::Color::Yellow);
    titleText.setPosition(300, 1);

    std::string instructions =
        "Use 'C' to add a Node\n\n"
        "Left Mouse Click on Node to Move the Node around\n\n"
        "Get near the node and press 'S' to get snapped and make an Edge\n\n"
        "Left Ctrl and move mouse to select the whole area\n\n"
        "Middle Mouse Scroll button to move the selected portion to any space\n\n"
        "Scroll Middle Mouse to zoom in and zoom out (panning)\n\n"
        "Select DFS or BFS and Press left shift + left mousebutton to start traversing\n\n"
        "Green edge and Cyan color nodes are visited\n\n"
        "Press any key to continue...";

    introText.setString(instructions);
    introText.setCharacterSize(30);
    introText.setFillColor(sf::Color(255, 255, 255, 220));
    introText.setPosition(300, 80);
    ...
    while (true) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed || sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
                return;
            }
        }
